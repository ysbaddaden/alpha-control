...
UI.Sortable.prototype.start = function (event) {
    ...
    if (item) {
        ...
        this.dragged.classList.add('ui-dragged');
        ...
        if (target) {
            ...
            if (idx !== undefined) {
                ...
                if (idx > this.draggedIdx && y > this.previousY) {
                    parent.after(this.dragged, target);
                    this.draggedIdx = idx;
                } else if (idx < this.draggedIdx && y < this.previousY) {
                    parent.before(this.dragged, target);
                    this.draggedIdx = idx;
                }
            }
        }
...
UI.Sortable.prototype.drop = function (event) {
    if (this.dragged) {
        if (this.dispatchEvent('drop') === false) {
            return;
        }

        this.dragged.classList.remove('ui-dragged');
        this.dragged = null;
